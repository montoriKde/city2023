...
bool hantei(vector<vector<double>>& vz,vector<vector<int>>& used,int j,int i,int tmp){//地面建物判定
...
void write(const char *Filename){//引数のデータを処理し、jimentest.txtに出力する関数
...
int main(int argc,char* argv[]){
    if(argc!=2){
        cout<<"コマンドライン引数に地図データファイルを一つ入力してください\n";
        exit(1);
    }
